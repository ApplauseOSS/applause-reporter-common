...
const WINSTON_DEFAULT_LOG_FORMAT = winston__namespace.format.printf(({ level, message, label, timestamp }) => {
    return `${timestamp} [${label}] ${level}: ${message}`;
});
function constructDefaultLogger() {
    return winston__namespace.createLogger({
        format: winston__namespace.format.combine(winston__namespace.format.label({ label: 'Applause Tests' }), winston__namespace.format.timestamp(), winston__namespace.format.splat(), WINSTON_DEFAULT_LOG_FORMAT),
        transports: [
            new winston__namespace.transports.File({ filename: 'error.log', level: 'error' }),
            new winston__namespace.transports.File({ filename: 'combined.log' }),
            new ApplauseTransport(),
            new winston__namespace.transports.Console({
                level: 'info',
                format: winston__namespace.format.combine(winston__namespace.format.colorize(), WINSTON_DEFAULT_LOG_FORMAT),
            }),
        ],
    });
}
...
exports.APPLAUSE_LOG_RECORDS = APPLAUSE_LOG_RECORDS;
exports.ApplauseReporter = ApplauseReporter;
exports.ApplauseTransport = ApplauseTransport;
exports.AutoApi = AutoApi;
exports.EmailHelper = EmailHelper;
exports.Inbox = Inbox;
exports.LoggingContainer = LoggingContainer;
...
exports.WINSTON_DEFAULT_LOG_FORMAT = WINSTON_DEFAULT_LOG_FORMAT;
exports.constructDefaultLogger = constructDefaultLogger;
